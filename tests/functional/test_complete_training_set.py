#!/usr/bin/env python3
"""
Complete Training Set Test
完整训练集测试 - 使用真实API测试较大规模的完整数据集
"""
...
class CompleteTrainingSetTest:
    ...
    async def run_training_set_test(
        ...
        results = {
            "test_info": {
                "test_name": "Complete Training Set Test",
    ...
    def save_results(self, results: Dict[str, Any], filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"complete_training_set_{dataset_name}_{sample_size}samples_{timestamp}.json"

        # 创建结果目录
        results_dir = Path("logs/training_set_results")
...
async def main():
    ...
    # 测试配置选项
    test_options = {
        "1": {"dataset": "arc_easy", "size": 50, "budget": 3.0, "desc": "ARC-Easy数据集 (50样本)"},
    ...
    # 由于Google API配额限制，使用较小的测试配置
    selected_config = {
        "dataset": "mixed",
        "size": 30,
        "budget": 2.0,
        "desc": "双模型混合数据集 (30样本) - 适应配额限制",
    }
    print(f"\n🔬 运行测试配置: {selected_config['desc']}")
    print(f"  注意: Google API配额已用完，将使用OpenAI + Anthropic进行双模型对比")

    # 创建测试实例
    tester = CompleteTrainingSetTest()
    ...
    # 运行测试
    results = await tester.run_training_set_test(
